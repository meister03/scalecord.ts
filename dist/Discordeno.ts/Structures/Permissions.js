...
class Permissions {
    ...
    static ALL = Object.values(Permissions.FLAGS).reduce((all, p) => all | p, 0n);
    bits;
    ...
    has(permission, checkAdministrator = true) {
        return (checkAdministrator && this._has(Permissions.FLAGS.ADMINISTRATOR)) || this._has(permission);
    }
    transform(bit) {
        // @ts-expect-error
        if (bit?.bits !== undefined)
            bit = bit.bits;
        const defaultBit = BigInt(0);
        if (Array.isArray(bit))
            return bit.map((p) => this.transform(p)).reduce((prev, p) => prev | p, defaultBit);
        if (typeof defaultBit === typeof bit && bit >= defaultBit)
            return bit;
        if (typeof bit === "string") {
            if (typeof Permissions.FLAGS[bit] !== "undefined")
                return Permissions.FLAGS[bit];
            if (!isNaN(bit))
    ...
    add(...bits) {
        ...
        if (Object.isFrozen(this))
            return new Permissions(this.bits | total);
        this.bits |= total;
        return this;
    }
    remove(...bits) {
